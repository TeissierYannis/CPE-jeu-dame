 ...
 *
 * Programme de test des classes du server.model
...
//
//		System.out.println("\nTest classe Model");
//		Model server.model = new Model();	// constructeur crée server.model et l'affiche
//		System.out.println("isPieceMoveable ('b',4) -> ('c',5)  = " 
//							+ server.model.isPieceMoveable(new Coord('b',4), new Coord('c',5)));	// true
//		System.out.println("isPieceMoveable ('c',7) -> ('d',6) = " 
//							+ server.model.isPieceMoveable(new Coord('c',7),  new Coord('d',6)));	// false, tour des blancs
//		System.out.println("isPieceMoveable ('c',3) -> ('d',4)  = " 
//							+ server.model.isPieceMoveable(new Coord('c',3),  new Coord('d',4)));	// false, case occupée
//		System.out.println("isPieceMoveable ('b',4) -> ('w',12) = " 
//							+ server.model.isPieceMoveable(new Coord('b',4),  new Coord('w',12)));	// false, hors damier
//
//		System.out.println("isMovePiecePossible ('b',4) -> ('c',5) = " + 
//							server.model.isMovePiecePossible(new Coord('b',4), new Coord('c',5), false));	// true
//		System.out.println("movePiece ('b',4) -> ('c',5) = move OK - Cf. damier ci-dessous" );
//							server.model.movePiece(new Coord('b',4), new Coord('c',5));	// move OK visible sur affichage
//		System.out.println(server.model);
//	
//		server.model.switchGamer(); // Changement joueur - c'est au tour des noirs
//		System.out.println("isPieceMoveable ('c',7) -> ('d',6) = " + 
//							server.model.isPieceMoveable(new Coord('c',7), new Coord('d',6)));	// true c'est bien au joueur noir de jouer
//		System.out.println("isMovePiecePossible ('c',7) -> ('c',6) = " + 
//							server.model.isMovePiecePossible(new Coord('c',7), new Coord('c',6), false));	// false pas déplacement en diagonale
//	
//		
//		// on recrée un objet Model sur lequel aucun mouvement n'a été effectué 
//		// pour tester méthode publique moveCapturePromote()
//		// normalement elle doit étre fonctionnelle sans modification si vous avez bien testé tout ce qui précéde
...
//							server.model.moveCapturePromote(new Coord('b',4), new Coord('c',5));
//		System.out.println("moveCapturePromote ('e',7) -> ('d',6)  = move OK - Cf. damier ci-dessous ");
//				server.model.moveCapturePromote(new Coord('e',7), new Coord('d',6));
//		System.out.println("moveCapturePromote ('c',5) -> ('e',7)  = move KO on ne gére pas encore les prises - Cf. damier ci-dessous ");
//				server.model.moveCapturePromote(new Coord('c',5), new Coord('e',7));
//		System.out.println("moveCapturePromote ('h',4) -> ('h',5)  = move KO déplacement vertical - Cf. damier ci-dessous ");
//				server.model.moveCapturePromote(new Coord('h',4), new Coord('h',5));
